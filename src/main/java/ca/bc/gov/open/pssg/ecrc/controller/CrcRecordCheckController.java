 ...
 */
@Controller
public class CrcRecordCheckController {
	...
	@RequestMapping(value="/secure/crcRecordCheckForm.htm", method = RequestMethod.GET)
	public String showForm(HttpServletRequest request, Map<String, Object> model) throws RestServiceException, ECRCException {
		...
		model.put("currentPage", "crcRecordCheckForm");
		
		String titlePrefix = sessionVars.getEcrc_props().getProperty("title.crc");
		String pageTitle = sessionVars.getEcrc_props().getProperty("title.crc.data.entry");
		...
		request.setAttribute("si_p", sessionVars.getEcrc_props().getProperty("wt.si_p.crc.data.entry"));
		
		return "crcRecordCheckForm";
	}
	
	// Process the form.
	@RequestMapping(value="/secure/crcRecordCheckForm.htm", method = RequestMethod.POST)
	public String processForm(@Validated({Default.class, RecordCheckForm.CrcChecks.class}) RecordCheckForm recordCheckForm, BindingResult result, Map<String, Object> model) throws Exception {
		
// 		Removed code to satisfy PT 22780		
//		String scheduleType = "";
//		if (sessionVars.getOrgApplRelationship().equals(Consts.ORG_APPL_RELATION_ONETIMETIK)) {
//			if (recordCheckForm.getScheduleType() == null || recordCheckForm.getScheduleType().isEmpty()) {
//				result.rejectValue("scheduleType", null, val_props.getProperty("scheduleType.required"));
//			} else {
//				scheduleType = recordCheckForm.getScheduleType();
//			}
//		} else {
//			scheduleType = sessionVars.getAuthenticateUser().getAccessCodeResponse().getDefaultScheduleTypeCd();
//		}
		
		// if schedule D, care facility is required
		if ((sessionVars.getRecordCheckForm().getScheduleType().equals(Consts.SCHEDULE_D) || sessionVars.getRecordCheckForm().getScheduleType().equals(Consts.SCHEDULE_OT_D))
				&& (recordCheckForm.getCareFacility() == null || recordCheckForm.getCareFacility().isEmpty()) ) {
			result.rejectValue("careFacility", null, val_props.getProperty("careFacility.required"));
		}
		...
		return "redirectCrcReviewDetails";
	}

	@RequestMapping(value="/secure/crcRecordCheckFormCancel.htm", method = RequestMethod.POST)
